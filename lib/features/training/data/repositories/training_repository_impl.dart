...
class TrainingRepositoryImpl implements TrainingRepository {
  ...
  Stream<List<ExerciseEntity>> watchExercises() {
    return database.watchAllExercises().map((exercises) {
      return exercises.map((e) => ExerciseEntity(
        id: e.id,
        name: e.name,
    ...
    return database.select(database.routines).watch().asyncMap((routinesList) async {
       ...
       for (final routineData in routinesList) {
         ...
         final exercisesQuery = database.select(database.routineExercises).join([
            innerJoin(database.exercises, database.exercises.id.equalsExp(database.routineExercises.exerciseId)),
  ...
  @override
  Future<Either<Failure, int>> addRoutine(String title, List<RoutineExerciseEntity> routineExercises, int? estDuration) async {
    try {
      return await database.transaction(() async {
        final routineId = await database.into(database.routines).insert(RoutinesCompanion(
          title: Value(title),
  ...
  @override
  Future<Either<Failure, void>> updateRoutine(int id, String title, List<RoutineExerciseEntity> exercises, int? estDuration) async {
    try {
      await database.transaction(() async {
        // Update routine metadata
        await (database.update(database.routines)..where((t) => t.id.equals(id))).write(
          RoutinesCompanion(
            title: Value(title),
            estimatedDuration: Value(estDuration),
          ),
        ...
        await (database.delete(database.routineExercises)..where((t) => t.routineId.equals(id))).go();
  ...
  Stream<List<WorkoutSetEntity>> watchWeightLogs() {
    return database.watchLatestWeightLogs().map((sets) {
      return sets.map((s) => WorkoutSetEntity(
        id: s.id,
  ...
  Stream<List<BodyWeightLogEntity>> watchBodyWeightLogs() {
    return database.watchLatestWeightEntries().map((logs) {
      return logs.map((l) => BodyWeightLogEntity(
        id: l.id,
        weight: l.weight,
        date: l.date,
      ),).toList();
