...
void main() {
  ...
  test('la migrazione ripristina una colonna mancante su cardio_sessions',
      () async {
    ...
    await firstRun
        .customStatement('ALTER TABLE cardio_sessions DROP COLUMN steps');
    expect(
    ...
    await secondRun.into(secondRun.cardioSessions).insert(
  ...
  test('ogni colonna dello schema corrente sopravvive a una riapertura',
      () async {
    final firstRun = AppDatabase(NativeDatabase(dbFile));
    final expected = <String, Set<String>>{};
    for (final table in firstRun.allTables) {
      expected[table.actualTableName] =
          await columnsOf(firstRun, table.actualTableName);
    }
    await firstRun.close();
