...
- Il database Drift locale vive nella directory documenti dell'app come `gym_db.sqlite` ed e' cifrato con SQLCipher. La chiave sta nel secure storage; `lib/core/database/connection.dart` gestisce apertura, generazione chiave e conversione dei database in chiaro creati da versioni precedenti. SQLCipher arriva da `package:sqlite3` 3.x tramite i build hook, scelto in `pubspec.yaml` con `hooks: user_defines: sqlite3: source: sqlcipher`: senza quella sezione verrebbe collegato SQLite normale. Non aggiungere `sqlite3_flutter_libs` ne' `sqlcipher_flutter_libs`, che con la versione 3 non servono piu' (il secondo e' dismesso), e non togliere il controllo su `PRAGMA cipher_version` in `connection.dart`: e' l'unica cosa che ferma l'app invece di lasciarla girare in chiaro.
- `AppDatabase` usa attualmente `schemaVersion => 21` e fa seed dei dati iniziali quando la tabella esercizi e' vuota.
