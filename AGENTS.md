...
- Per l'etichetta in maiuscolo che introduce un gruppo di contenuti usa `SectionTitle` (`lib/core/widgets/section_title.dart`) con uno dei due toni: `primary` per il titolo di una sezione, `muted` per l'etichetta sopra un gruppo di campi. Corpo, peso e spaziatura non si scelgono.
